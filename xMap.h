...
xMap<K, V>::xMap(int (*hashCode)(K&, int), float loadFactor, bool (*valueEqual)(V& lhs, V& rhs),
         ...
         hashCode(hashCode), loadFactor(loadFactor),
         valueEqual(valueEqual), deleteValues(deleteValues), 
         keyEqual(keyEqual), deleteKeys(deleteKeys) {
  this->capacity = 10;  
  this->count = 0;
...
V xMap<K, V>::put(K key, V value) {
  ...
  for (typename DLinkedList<Entry*>::Iterator pEntry = list.begin(); pEntry != list.end(); ++pEntry) {
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K)) {
  ...
  for (typename DLinkedList<Entry*>::Iterator pEntry = list.begin(); pEntry != list.end(); ++pEntry) {
    if (keyEQ((*pEntry)->key, key)) {
      ...
      if (deleteKeyInMap) { deleteKeyInMap((*pEntry)->key); }
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K),
  ...
  for (typename DLinkedList<Entry*>::Iterator pEntry = list.begin(); pEntry != list.end(); ++pEntry) {
    if (keyEQ((*pEntry)->key, key) && valueEQ((*pEntry)->value, value)) {
      V retValue = (*pEntry)->value;
      if (deleteKeyInMap) { deleteKeyInMap((*pEntry)->key); }
      if (deleteValueInMap) { deleteValueInMap((*pEntry)->value); }
      list.removeItem(*pEntry, xMap<K,V>::deleteEntry);
      this->count--;
