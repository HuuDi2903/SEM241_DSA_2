...
Heap<T>::Heap(int (*comparator)(T&, T&), void (*deleteUserData)(Heap<T>*)) : 
              comparator(comparator), deleteUserData(deleteUserData) {
  // YOUR CODE IS HERE
  this->capacity = 10;
  this->count = 0;
...
T Heap<T>::pop() {
  ...
  this->elements[0] = this->elements[count - 1];
  count--;
  reheapDown(0);
...
void Heap<T>::reheapUp(int position) {
  ...
  while (position > 0 && compare(elements[position], elements[parent]) < 0) {
...
void Heap<T>::reheapDown(int position) {
  ...
  if (leftChild < count && compare(elements[leftChild], elements[largest]) < 0) {  // compare = -1 => a < b
    largest = leftChild;
  }
...
int Heap<T>::getItem(T item) {
  ...
  for (int i = 0; i < count; i++) {
    if (compare(elements[i], item) == 0) {  // compare = 0  ->  a == b
      return i;
    }
  }
...
void Heap<T>::copyFrom(const Heap<T>& heap) {
  ...
  // this->deleteUserData = heap.deleteUserData;  // teacher
