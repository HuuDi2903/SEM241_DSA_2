...
∗ Computes the response according to Formula 12. Note that do not add b if bias
is not used. It is recommended to use the function xt::linalg::tensordot to calculate XWT in Formula 12.

For a single data sample: y = Wx + b
For a batch of data: Y = XW(T) + b (broadcasting)
*/
...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
    ...
    // m_aWeights = N_out x N_in = W
    if (X.shape().size() == 1) {
        // Y = matmul_on_stack(this->m_aWeights, X);
        // Y = xt::linalg::dot(this->m_aWeights, X); //0.72

        Y = xt::linalg::tensordot(this->m_aWeights, X, 1);  
        // cout << "Y 1D shape: " << xt::adapt(Y.shape()) << std::endl;
    } else {
        // cout << "Weights shape: " << xt::adapt(this->m_aWeights.shape()) << std::endl;
        Y = xt::linalg::tensordot(X, this->m_aWeights, {1}, {1});  // 3-2 x 2-3 = 3-3
        // cout << "Y 2D shape: " << xt::adapt(Y.shape()) << std::endl;
    }
...
/*
– Input: xt::xarray<double> DY. DY is the derivative of the loss function with respect
to the output of FCLayer.
– Performs:
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
    //YOUR CODE IS HERE
    this->m_unSample_Counter += DY.shape()[0];  //  keep track of the number of samples in the batch
 
    xt::xarray<double> dW_batch;
    // cout << "DY shape: " << xt::adapt(DY.shape()) << std::endl;
    // cout << "X shape: " << xt::adapt(this->m_aCached_X.shape()) << std::endl;

    if (DY.shape().size() == 1) {
        // Single sample case
        this->m_aGrad_W = xt::linalg::outer(DY, xt::transpose(this->m_aCached_X));
        // cout << "Grad_W 1D shape: " << xt::adapt(this->m_aGrad_W.shape()) << std::endl;
    } else {
        // Batch case
        dW_batch = xt::zeros<double>({DY.shape()[0], m_aWeights.shape()[0], m_aWeights.shape()[1]});
        
        // for (size_t i = 0; i < DY.shape()[0]; ++i) {

        // }
        dW_batch = outer_stack(DY, this->m_aCached_X);

        // this->m_aGrad_W = xt::sum(dW_batch, {0}) / DY.shape()[0]; //0.72
        this->m_aGrad_W = xt::sum(dW_batch, {0});
        // cout << "Grad_W 2D shape: " << xt::adapt(this->m_aGrad_W.shape()) << std::endl;  // 3-3 = 3-2 x 2-3
    }
    ...
    if (this->m_bUse_Bias) {
        ...
        if (DY.shape().size() > 1) {
            this->m_aGrad_b /= DY.shape()[0];  // Average over batch size
        }
    }
